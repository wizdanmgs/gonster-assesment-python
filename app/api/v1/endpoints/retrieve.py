...
async def get_machine_data(
    ...
    start_time: datetime = Query(
        ..., description="ISO 8601 Datetime for start of window"
    ),
    end_time: datetime = Query(..., description="ISO 8601 Datetime for end of window"),
    interval: Optional[str] = Query(
        None, description="Aggregation interval e.g., '1m', '1h', '1d'"
    ),
    ...
    if start_time >= end_time:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=MSG_INVALID_TIME_RANGE
        )
