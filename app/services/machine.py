...
async def create_machine(
    repo: MachineRepository, machine_in: MachineCreate
) -> MachineMetadata:
    return await repo.create(machine_in)


async def get_machine(
    repo: MachineRepository, machine_id: uuid.UUID
) -> Optional[MachineMetadata]:
...
async def validate_machines_exist(
    repo: MachineRepository, machine_ids: List[uuid.UUID]
) -> List[uuid.UUID]:
    """
